...
public class NotifyDatabase extends SQLiteOpenHelper {
	...
	private void clear(long time){
		...
		SQLiteDatabase db = getWritableDatabase();
		db.delete(TBL_NAME, DAY + "<" + time + " AND " + 
				CATEGORY + " = " + RepeatCategory.NONE.getId(), null);
		db.close();
	}
	public List<NotificationData> query(long time){
		...
		do{
			...
			n.setLocation(loc ++);
			ByteArrayInputStream input = new ByteArrayInputStream(c.getBlob(c.getColumnIndex(BMP)));
			Bitmap bmp = null;
			try{
